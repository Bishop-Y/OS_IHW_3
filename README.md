...
> * В третьем терминале: 8 раз прописать ```./client <k>```, где k - сумма наследства текущего наследника. <br>
...
> * В третьем терминале: 8 раз прописать ```./client <k> <IP> <Port>```, где k - сумма наследства текущего наследника, IP - IP-адрес, и Port - порт. <br>
...
1) Стандартный запуск: <br>
   ...
   А каждому наследнику нужно будет только прописать сумму его наследства.
> * В первом терминале: ```./server```
> * Во втором терминале: ```./client <k>```, где k - сумма наследства текущего наследника. <br>
> * Во других терминалах: ```./monitor``` (причём можно это сделать в любой момент времени)
...
> * Во втором терминале: 8 раз прописать ```./client <k> <IP> <Port_client>```, где k - сумма наследства текущего наследника, IP - IP-адрес, и Port_client - порт клиентов.
> * В других терминалах: ```./monitor <IP> <Port_monitor>``` (причём можно это сделать в любой момент времени), где IP - IP-адрес, и Port_monitor - порт мониторов. <br>
